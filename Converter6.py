"""
converter6.py: A script to convert PDF files to text using OCR
Number 6 in the series of scripts to convert PDF files to text using OCR.
...
This version will send the text to ChatGPT for corrections.
...
# Load the OpenAI API key from the .env file in the same folder See README.md for more information
load_dotenv()
...
with open(file_path, 'rb') as pdfFileObj:
    ...
    with open('output_raw.txt', 'w', encoding='utf-8') as raw_output_file, open('output_corrected.txt', 'w', encoding='utf-8') as corrected_output_file:
        for page_num in range(num_pages):
            images = convert_from_path(file_path, first_page=page_num+1, last_page=page_num+1)

            # Iterate through all the pages stored above and extract the text from each image page & get ChatGPT to correct the text
            for image in images:
    ...
    # Print a newline of characters to move the cursor to the next line
    print()
